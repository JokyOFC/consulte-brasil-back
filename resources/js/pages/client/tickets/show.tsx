...
export default function ClientTicketShow() {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.post(`/client/tickets/${ticket.id}/reply`, {
            forceFormData: true,
    ...
    return (
        <>
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
                <PageHeader
                    title={ticket.title}
                    description={`${ticket.category_label} · aberto em ${formatDateTime(ticket.created_at)}`}
                    actions={
                        <div className="flex items-center gap-2">
                            <Badge variant="outline">{ticket.status_label}</Badge>
                            <Button variant="ghost" asChild>
                                <Link href="/client/tickets">Voltar</Link>
                            </Button>
                        </div>
                    }
                />

                <Card className={cn('border-l-4 border-l-muted')}>
                    <CardContent className="space-y-2 p-5">
                        <div className="text-xs font-medium text-muted-foreground">Descrição original</div>
